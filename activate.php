...
if (isset($_GET['id'])){
    ...
    if (isset($_GET['choice'])) {
        if ($_GET['choice'] == 'yes') {
            $stmt = $db->prepare('UPDATE polls SET status = 1 WHERE qid = ?');
            $stmt->execute([ $_GET['id'] ]);
          
            $msg = 'You have activate the poll!';
        } else {
    ...
    exit('Error You are no authorized to activate this poll!!');
}
...
<div class="container deactivatepoll">
	<h2>activate Poll :<?=$poll['question']?></h2>
    <?php if (isset($msg)): ?>
    <p><?=$msg?></p>
    ...
    <?php else: ?>
	<p>Are you sure you want to activate poll <?=$poll['question']?></p>
    <div class="choices">
        <a href="activate.php?id=<?=$poll['qid']?>&choice=yes">Yes</a>
        <a href="activate.php?id=<?=$poll['qid']?>&choice=no">No</a>
    </div>
    <?php endif; ?>
</div>
